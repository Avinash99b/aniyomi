...
object ProxyServer {
        ...
        server = embeddedServer(CIO, port = 8080) {
            routing {
                get("/chunk/{segmentIdx}") {
                    ...
                    val cacheData =
                        sessions[URLEncoder.encode(videoUrl, "UTF-8")]
    ...
    fun generateProxyChunkUrl(videoUrl: String, segmentIdx: Int): String {
        ...
        return "$BASE_URL/chunk/$segmentIdx?videoUrl=$encodedUrl"
    }

    fun getStreamFileUrl(absolutePath: String): String {
        val encodedPath = URLEncoder.encode(absolutePath, "UTF-8")
        return "$BASE_URL/stream_file?filePath=$encodedPath"
    }
    ...
    fun generateProxyDirectUrl(videoUrl: String, orgUrl: String): String {
        ...
        return "$BASE_URL/direct_video.mp4?videoUrl=$encodedVideoUrl&orgUrl=$encodedOrgUrl"
    }
}
